...
parts of the reference site may inform Forge, which parts require configuration,
and which parts must never enter a generated project.
...
implements a `reusable-core` or `configurable-example` row, its pull request
must add a provenance entry containing:
